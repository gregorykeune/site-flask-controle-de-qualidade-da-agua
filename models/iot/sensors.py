...
class Sensor(db.Model):
    ...
    def save_sensor(name, brand, model, unit, topic, is_active):
    ...
    def get_all_sensors():
        ...
        if sensors is not None:
            sensors = Sensor.query.join(Device).add_columns(Device.id, Device.name, Device.brand,
            Device.model, Device.is_active, Sensor.topic, Sensor.unit).all()
            return sensors
        
    def update_sensor(id, name, brand, model, topic, unit, is_active):
        device = Device.query.filter(Device.id == id).first()
        sensor = Sensor.query.filter(Sensor.devices_id == id).first()
        if device is not None and sensor is not None:
            device.name = name
            device.brand = brand
            device.model = model
            sensor.topic = topic
